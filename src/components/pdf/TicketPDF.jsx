...
export const TicketPDF = ({ ticketData, qrCodeImage, brandLogoImage }) => {
  ...
  const formatFullAddress = (location) => {
    ...
    // Verifica o tipo (Aceita 'Physical' OU 'presential' conforme seu JSON)
    const isPhysical = location.type === 'Physical' || location.type === 'presential';
    
    // Se não for presencial ou não tiver endereço, retorna padrão
    if (!isPhysical || !location.address) { 
      return "Local a definir"; 
    }

    const { venueName, address } = location;
    
    // Garante que address existe antes de acessar as propriedades para evitar crash
    ...
    // Se tiver nome do local, inclui, senão começa pelo endereço
