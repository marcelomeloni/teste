...
const getSalesStartTimestamp = (event) => {
    // Priorizar sales_start_date direto do evento
    if (event.sales_start_date) {
        return event.sales_start_date;
    }
    
    // Fallback para account.salesStartDate
    ...
    // Se for string de data, converter para timestamp
    if (typeof salesStartDate === 'string') {
        return Math.floor(new Date(salesStartDate).getTime() / 1000);
    }
...
const getEventDate = (event) => {
    // Prioriza event_start_date direto do evento
    if (event.event_start_date) {
        return new Date(event.event_start_date);
    }
    
    // Fallback para account.eventStartDate
    if (event.account?.eventStartDate) {
        return new Date(event.account.eventStartDate);
    }
...
export function Events() {
    ...
    // Buscar eventos
    // Na página Events, modifique o useEffect:
    useEffect(() => {
        const fetchEvents = async () => {
            ...
            try {
                ...
                // ✅ PROCESSAMENTO CORRIGIDO - Estrutura compatível com EventCard
                const processedEvents = await Promise.all(
                    eventsData.map(async (event) => {
                        ...
                        const eventWithMetadata = { 
                            ...
                            publicKey: event.event_address, // Para compatibilidade
                            account: {
                                // Campos que o EventCard espera
                                name: event.name,
                                ...
                                salesStartDate: event.sales_start_date,
                        ...
                        // Se metadata_uri é uma URL IPFS, buscar o conteúdo
                        if (event.metadata_uri && typeof event.metadata_uri === 'string' && event.metadata_uri.startsWith('http')) {
                            try {
                                console.log(`🌐 Buscando metadata para: ${event.name}`);
                                const metadataResponse = await fetch(event.metadata_uri);
                                if (metadataResponse.ok) {
                                    const metadata = await metadataResponse.json();
                                    
                                    // Mesclar dados do metadata com o evento
                                    if (metadata) {
                                        // Atualizar campos básicos se estiverem faltando
                                        if (!eventWithMetadata.account.name && metadata.name) {
                                            eventWithMetadata.account.name = metadata.name;
                                        }
                                        if (!eventWithMetadata.account.description && metadata.description) {
                                            eventWithMetadata.account.description = metadata.description;
                                        }
                                        if (metadata.location?.address?.city) {
                                            eventWithMetadata.account.locationName = metadata.location.address.city;
                                            eventWithMetadata.location_name = metadata.location.address.city;
                                        }
                                        if (metadata.category && !eventWithMetadata.metadata) {
                                            eventWithMetadata.metadata = { category: metadata.category };
                                        } else if (metadata.category) {
                                            eventWithMetadata.metadata.category = metadata.category;
                                        }
                                        if (!eventWithMetadata.account.imageUrl && metadata.image) {
                                            eventWithMetadata.account.imageUrl = metadata.image;
                                        }
    
                                        // Processar tiers se existirem
                                        if (metadata.ticketing?.tiers) {
                                            const processedTiers = metadata.ticketing.tiers.map(tier => {
                                                const priceBrlCents = tier.price ? Math.round(tier.price * 100) : 0;
                                                const maxTicketsSupply = tier.quantity || 0;
                                                
                                                return {
                                                    name: tier.name,
                                                    priceBrlCents: priceBrlCents,
                                                    maxTicketsSupply: maxTicketsSupply,
                                                    ticketsSold: 0, // Não temos essa informação
                                                    isSoldOut: false,
                                            ...
                                            eventWithMetadata.account.tiers = processedTiers;
                                            eventWithMetadata.account.maxTotalSupply = processedTiers.reduce((sum, tier) => sum + tier.maxTicketsSupply, 0);
                                        }
    
                                        // Adicionar location se disponível
                                        if (metadata.location && !eventWithMetadata.account.locationName) {
                                            eventWithMetadata.account.locationName = metadata.location.venueName;
                                        }
    
                                        // Adicionar datas se disponíveis
                                        if (metadata.dateTime) {
                                            if (!eventWithMetadata.account.eventStartDate && metadata.dateTime.start) {
                                                eventWithMetadata.account.eventStartDate = metadata.dateTime.start;
                                            }
                                            if (!eventWithMetadata.account.eventEndDate && metadata.dateTime.end) {
                                                eventWithMetadata.account.eventEndDate = metadata.dateTime.end;
                                            }
                                        }
    
                                        console.log(`✅ Metadata carregado para: ${event.name}`);
                                    }
                                }
                            } catch (error) {
                                console.error(`❌ Erro ao buscar metadata para ${event.name}:`, error);
                            }
                        }
    
                        // Garantir que todos os campos obrigatórios existem
                        if (!eventWithMetadata.account.name) eventWithMetadata.account.name = 'Evento sem nome';
                        if (!eventWithMetadata.account.description) eventWithMetadata.account.description = 'Descrição não disponível';
                        if (!eventWithMetadata.account.imageUrl) eventWithMetadata.account.imageUrl = '/default-event-image.jpg';
    
                        return eventWithMetadata;
                    })
                );
                
                console.log(`✅ ${processedEvents.length} eventos carregados e processados`);
    ...
    const getEventCity = (event) => {
        // Tentar pegar cidade do metadata
        if (event.metadata?.location?.address?.city) {
            return event.metadata.location.address.city;
        }
        
        // Tentar extrair cidade do location_name
        const locationName = event.location_name || event.account?.locationName || '';
        ...
        return locationName || 'Online';
    };
    // Extrair categorias únicas
    const categories = useMemo(() => {
        const cats = events
            .map(event => {
                // Tentar pegar categoria do metadata se disponível
    ...
    const locations = useMemo(() => {
        const cities = events
            ...
            .filter((city, index, self) => self.indexOf(city) === index && city);
        return ['all', ...cities];
    }, [events]);
    ...
    useEffect(() => {
        ...
        if (searchTerm) {
            results = results.filter(event => {
                const eventName = event.name || event.account?.name || '';
                const eventDescription = event.description || event.account?.description || '';
                const eventCategory = event.metadata?.category || 'Geral';
                
                return eventName.toLowerCase().includes(searchTerm.toLowerCase()) ||
                       eventDescription.toLowerCase().includes(searchTerm.toLowerCase()) ||
                       eventCategory.toLowerCase().includes(searchTerm.toLowerCase());
            });
        }
    
        // Filtro de categoria - ATUALIZADO
        ...
        results.sort((a, b) => {
            switch (sortBy) {
                ...
                case 'name':
                    const aName = a.name || a.account?.name || '';
                    const bName = b.name || b.account?.name || '';
    ...
    const stats = {
        total: events.length,
        online: events.filter(e => {
            // Verificar se é online baseado na cidade
            const city = getEventCity(e);
            return !city || city === 'Online' || city.toLowerCase().includes('online');
        }).length,
    ...
    <EventCard key={event.event_address} event={event} />
))}
                    </div>
                ) : (
